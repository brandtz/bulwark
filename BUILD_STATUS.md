...
E3-S1 — Properties pipeline list view (start of E3 — Properties).

E2-S8 closed: persona × route matrix. New `tests/e2e/persona-matrix.spec.ts` data-drives a `(persona, route, outcome)` table; 10 tests cover `/admin/dashboard` and `/admin/properties` against {admin, super_admin, field, sub, anonymous} — admin/super reach the page, field/sub bounce to `/403`, anonymous bounces to `/login?next=...`. Adding a new protected route is a one-line change. **48 chromium tests passing** (1 skipped) + 6 unit tests. **Epic E2 complete.**
...
1. **E3-S1** — Properties pipeline list view.
2. E3-S2 — Property detail view + status flow.
