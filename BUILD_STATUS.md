...
E3-S2 — Properties pipeline list view (mobile) with FT-12 segmented kanban/list toggle.

E3-S1 closed: properties pipeline kanban shipped at `/admin/properties`. New `app/components/property/PipelineColumn.vue` (header + count chip + empty state) and `PropertyCard.vue` (address + city/state + StatusBadge, deep-links to `/admin/properties/{id}`). Page server-renders the full property list (pageSize 100), groups client-side by status, and renders all 13 columns in a horizontal scroll strip — even empty ones, so end-of-pipeline statuses (cancelled) stay visible. Header shows total count + `New property` CTA. New `tests/e2e/properties-pipeline.spec.ts` (5 tests). **53 chromium tests passing** (1 skipped) + 6 unit tests.
...
1. **E3-S2** — Pipeline list view (mobile) + FT-12 toggle.
2. E3-S3 — Drag-drop status change (desktop) + long-press action sheet (mobile).
