...
E2-S8 — Playwright persona matrix (login each persona, assert allowed routes return 200, disallowed return 403).

E2-S7 closed: tenant firewall scaffolded into `MockServiceFactory`. New `shared/mocks/tenant.ts` exports `TenantContext`, `TenantResolver`, `TenantViolationError`, `assertSameTenant`. Each domain mock (`MockPropertyService`, `MockClientService`) takes a resolver in its constructor and asserts at the top of every method that takes an `organizationId` — cross-tenant calls throw `TenantViolationError` before touching the row store. Factory wires a default resolver via new `MockAuthService.resolveTenantSync()` (mirrors async `currentUser` for the active-org override). New `tests/unit/tenant-firewall.test.ts` (6 tests) proves the firewall throws on cross-tenant property.list / property.get / client.list and short-circuits when the resolver is missing or returns null. **38 chromium tests passing** (1 skipped) + 6 unit tests.
...
1. **E2-S8** — Playwright persona matrix.
2. E3-S1 — Properties pipeline list view.
